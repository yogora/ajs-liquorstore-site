...
- **Visit Us CTA** (`.visit-cta`): a standalone callout before `</main>`, deliberately *not* styled like the footer it sits above — the outer `.visit-cta` section keeps the page's cream `--off-white` background (not `--ink`), and the actual content lives in a `.visit-cta-panel`: a rounded, `--red`-bordered card using the dedicated `--maroon` token (a deep oxblood, distinct from both `--ink` and `--off-white`) so it visually pops as its own element instead of merging into the black footer below. Inside `.visit-cta-panel` is `.visit-cta-inner`, a two-column flex row: left side (`.visit-cta-content`) has a headline + `.visit-cta-desc` blurb reiterating that the page is just a sample of what's carried in-store, plus `.visit-cta-actions` with a `tel:` call button and a "Get Directions" link (reusing `.btn`/`.btn-primary`/`.btn-ghost`); right side (`.visit-cta-card`) is a translucent sub-panel with store hours (`.visit-cta-hours`) and the address, matching the footer's data — keep both in sync if hours or location ever change.
