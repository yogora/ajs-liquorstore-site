...
- `.placeholder-section` / `.placeholder-inner` — the "coming soon" body still used by `beer.html`, `wine.html`, and `ready-to-drink.html`, which have no real content yet. `liquor.html` has been built out (see **Product category pages** below) and no longer uses this pattern — use it as the template when building out the other three.
...
**Product category pages.** `liquor.html` is the built-out template for the four category pages; `beer.html`, `wine.html`, and `ready-to-drink.html` should follow the same structure with category names, subcategories, and products swapped in. The page is built from these pieces, in order:
...
- **Subcategory sections** (`.product-section`, one per subcategory, each with a semantic `id` like `#bourbon` — not `#section-1`, since these double as deep links). Each has one `<h2>` (never skip to `<h3>` without it), a one-sentence blurb in `.product-section-intro`, and a `.product-grid` of `.product-card`s (5 per subcategory on `liquor.html`).
